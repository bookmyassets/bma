...
const ProjectAmenities = () => {
  ...
  // On desktop (lg screens and above), always show all amenities
  // On mobile, show only 6 unless showAll is true
  ...
  return (
    <>
      <style>{`
        ...
        /* Desktop always shows all amenities */
      ...
      <div className="bg-white py-12 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            <p
              className="text-2xl md:text-3xl font-bold tracking-[0.25em] uppercase mb-3"
              style={{ color: "#deae3c" }}
            >
              WestWyn Estates
            </p>
          ...
          <div className="flex flex-col lg:flex-row gap-10 items-start">
            ...
            <div className="w-full lg:w-[50%]">
              {/* Mobile view: conditionally show amenities based on showAll */}
              <div className="block lg:hidden">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4">
                  {visibleAmenities.map((amenity, index) => (
                    <div
                      key={amenity.title}
                      className={`amenity-card border-2 rounded-2xl p-3 sm:p-4 flex flex-col items-center text-center`}
                      style={{ borderColor: "#e8e8e8" }}
                    >
                      <div
                        className={`card-icon flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${amenity.color} rounded-xl mb-2 sm:mb-4`}
                      >
                        <div className="text-white text-sm sm:text-base">{amenity.icon}</div>
                      </div>
                      <h3
                ...
                {amenities.length > 6 && (
                  <div className="flex justify-center mt-8">
                    <button
                      ...
                      className="show-btn bg-[#deae3c] text-white px-8 py-3 rounded-full font-semibold text-sm hover:bg-opacity-90 transition-all"
                    >
              ...
              {/* Desktop view: always show all amenities */}
